...
class OrderDetailSubsScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loadingOrder: true,
  ...
  alertFunc(subscription_id) {
    Alert.prompt(
      'Cancelar Suscripción',
      'Escriba el motivo por el cual desea cancelar su suscripción.',
      [
        {
          text: 'Regresar',
        ...
        },
        {
          ...
          onPress: text =>
            text.length > 0 ? this.cancel(subscription_id, text) : this.displayAlert(true)
        }
      ],
      'plain-text'
    );
  ...
  cancel(subscription_id, reason) {
    ...
    http
      .post('client/subscriptions/cancel', { subscription_id, reason_for_cancellation: reason })
      .then(result => {
        this.setState(
          {
            loadingOrder: false
          },
          () => this.getDataAll(result.msg)
        );
      })
      .catch(error => {
        alert(error);
  ...
  displayAlert(error, subscription_id) {
    if (error) {
      Alert.alert(
        'ATENCIÓN',
        'Es necesario que ingreses una razón',
        [{ text: 'Ok', onPress: () => this.alertFunc(subscription_id) }],
        { cancelable: false }
      );
    } else {
      this.alertFunc(subscription_id);
    }
  }

  onClick(subscription_id) {
    this.displayAlert(false, subscription_id);
              ...
              <View style={{ flex: 1 }}>
                <View
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    ...
                    <Text allowFontScaling={false}>{order.product.name}</Text>
                  </View>
                  <View style={{ flexDirection: 'row' }}>
                    <Text allowFontScaling={false} style={Style.boldText}>
                      Folio:{' '}
                    </Text>
                    <Text allowFontScaling={false}>{order.number_order}</Text>
                  </View>
                  <View style={{ flexDirection: 'row' }}>
                    <Text allowFontScaling={false} style={Style.boldText}>
                      Fecha de contratación:{' '}
                    </Text>
                    <Text>{dayjs(order.current_period_start).format('DD/MM/YYYY')}</Text>
                  </View>
                  <View style={{ flexDirection: 'row' }}>
                    <Text allowFontScaling={false} style={Style.boldText}>
                  ...
                  <View style={{ padding: 20 }}>
                    <View style={{ flexDirection: 'row' }}>
                      ...
                      <Text>
                        {order.status === 'canceled'
                          ? 'Cancelada'
                          : dayjs(order.current_period_end).format('DD/MM/YYYY')}
                      </Text>
                    </View>
                    <View style={{ flexDirection: 'row', marginTop: -5 }}>
                      <TouchableOpacity
                        ...
                        <View
                          ...
                          <Image
                            source={typeOfCard(JSON.parse(order.card).brand)}
                          ...
                          <Text
                            style={{
                              fontWeight: '500',
                              marginLeft: 3
                            }}>{` QUE TERMINA EN ${JSON.parse(order.card).last4}`}</Text>
                        </View>
                      </TouchableOpacity>
                    </View>
                    <View
                      ...
                      <TouchableOpacity
                        onPress={() => this.onChangePaymentMethod(order.subscription_stripe_id)}
                    ...
                    <View
                      ...
                      <SecondaryButton
                        ...
                        height={40}
                        borderRadius={20}
                        onPress={() => this.onClick(order.subscription_stripe_id)}
                      />
                    </View>
                    ...
                    <View
                      ...
                      {order.status === 'canceled' && (
                        <Text
                          ...
                          {`Está suscripción fue cancelada el día ${dayjs(
                            order.date_canceled
                          ).format('DD/MM/YYYY')} y tiene vigencia hasta el día ${dayjs(
                    ...
                    <View style={{ flex: 1 }}>
                      <View
                        ...
                        <Text
                          ...
                          {`${dayjs(order.current_period_end).format('DD/MM/YY')}`}
                        </Text>
                      */}
                      </View>
...
export default OrderDetailSubsScreen;
